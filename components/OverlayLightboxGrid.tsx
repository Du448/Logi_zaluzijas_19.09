    ...
    <div className={className}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
        {safeImages.map((src, i) => {
          // Per-tile interactive magnetic center circle
          ...
          const onMove = (e: React.MouseEvent<HTMLButtonElement>) => {
            ...
            // Attraction radius thresholds
            const nearRadius = Math.min(rect.width, rect.height) * 0.25 // start attracting when within 25% of shortest side
            const overRadius = Math.min(rect.width, rect.height) * 0.12 // consider as "over button" when very close
            setNear(dist < nearRadius)
            setOver(dist < overRadius)
            // Translate circle slightly toward cursor, max 12px
            const maxOffset = 12
          ...
          const onLeave = () => {
            setDx(0); setDy(0); setNear(false); setOver(false)
          }
          ...
          return (
            ...
            >
              ...
              {/* magnetic circular badge with 'view' */}
              <div className="absolute inset-0 flex items-center justify-center select-none">
                ...
                >
                  ...
                  {/* fill that grows on hover or when over */}
