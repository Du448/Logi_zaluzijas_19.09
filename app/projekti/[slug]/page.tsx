...
export function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  if (!item) return { title: 'Projekts' }
  return {
    title: item.title,
    description:
      item.excerpt ??
      `${item.title} - PVC logu nomaiņa un uzstādīšana ar Vestalux: cenas, specifikācijas un darbu galerija.`,
    alternates: { canonical: `/projekti/${item.slug}` },
    openGraph: item.image
      ? { title: item.title, images: [{ url: item.image, alt: item.title }] }
      : undefined,
  }
}
    ...
    <section className="section">
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: item.title, path: `/projekti/${item.slug}` },
        ])}
      />
